...
class Mtbot:
    ...
    def setup(self):
        self.app.add_handler(
            MessageHandler(
                self.handle_message, filters=filters.regex("#ultimiacquisti")
            )
        )

    def handle_message(self, client: Client, message: Message):
        custom_date_error = False
        self.logger.info("Received message from mtproto")
        ...
        if not result["error"]:
            self.purchase.add_purchase(user, price, message_id, chat_id, date)
